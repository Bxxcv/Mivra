...
export default function TierDonut({ free, premium }: { free: number; premium: number }) {
  ...
  const data = [
    ...
    { name: 'Gratis', value: free, color: isDark ? 'rgba(251,248,242,0.15)' : 'rgba(23,20,15,0.1)' },
  ];
  const total = free + premium;

  return (
    <div className="rounded-2xl bg-ink/5 p-5 dark:bg-white/[0.04]">
      <p className="text-[13px] font-bold text-ink dark:text-cream">Distribusi tier</p>
      <div className="relative mt-3 h-[170px]">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <p className="font-display text-[20px] font-bold text-ink dark:text-cream">
            {total > 0 ? Math.round((premium / total) * 100) : 0}%
          </p>
          <p className="text-[10px] font-medium text-ink-400 dark:text-cream/40">Premium</p>
        </div>
      </div>
    </div>
  );
