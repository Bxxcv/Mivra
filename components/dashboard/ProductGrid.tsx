  ...
  if (products.length === 0) {
    return (
      <div className="mt-4 rounded-2xl border border-dashed border-ink/15 bg-white p-8 text-center">
        <p className="text-[13.5px] text-ink-400">Belum ada produk. Tambahkan produk pertamamu di form atas.</p>
      </div>
    );
  }
    ...
    <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {products.map((p) => {
        ...
        return (
          <div
            key={p.id}
            className={`overflow-hidden rounded-2xl border border-ink/8 bg-white shadow-soft transition-opacity ${
              busy ? 'opacity-50' : ''
          ...
          >
            <div className="aspect-square w-full bg-cream-100">
              {p.images[0] ? (
                ...
                <div className="flex h-full w-full items-center justify-center text-[11px] text-ink-300">
                  Tanpa foto
                </div>
              )}
            </div>
            <div className="p-3">
              ...
              <p className="text-[10.5px] text-ink-400">{p.stock === null ? 'Stok tak terbatas' : `Stok: ${p.stock}`}</p>
              ...
              <div className="mt-2 flex gap-1.5">
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-ink/10 py-1.5 text-[10.5px] font-semibold text-ink-500 hover:bg-ink/5"
                >
